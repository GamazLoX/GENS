...
const c3_callFunction = self.c3_callFunction;
...
};
{
    C3.Behaviors.skymen_Skymen_SpritefontDX.Instance = class Skymen_SpritefontDXInstance extends C3.SDKBehaviorInstanceBase
    {
        ...
        getDefault(tag)
        {
            switch (tag)
            {
                ...
                case "background":
                    return '#FFFFFF';
                case "size":
                    if (C3.Plugins.Text && this._inst._objectType._plugin instanceof C3.Plugins.Text)
                    {
        ...
        GetNbNewlines(nb, text)
        {
            ...
            let nbLines = 0;
            while (lines.length > 1 && text.startsWith(lines[0].fragments[0].text))
            {
                nbLines++;
                text = text.slice(lines.shift().fragments[0].text.length).trimStart();
            }
        ...
        GetSetTexttAct() {
            if (C3.Plugins.Spritefont2 && this._inst._objectType._plugin instanceof C3.Plugins.Spritefont2)
            {
                return C3.Plugins.Spritefont2.Acts.SetText
            }
            else if (C3.Plugins.Text && this._inst._objectType._plugin instanceof C3.Plugins.Text)
            {
                return C3.Plugins.Text.Acts.SetText;
            }
            return null;
        }
